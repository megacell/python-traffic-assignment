...
def cost_ratio(flow, net, eps=1e-8):
    ...
    tmp = np.divide(np.maximum(cost(flow, net), eps),
                    np.maximum(net[:, 3], eps))
...
def gas_emission(speed):
    return 350. * (speed > 30.) + np.multiply(
        (speed <= 30.), 350. + 650. * (30. - speed) / 30.)


def compute_metrics(alpha, f, net, d, feat, subset, out, row, fs=None,
                    net2=None, length_unit='Mile', time_unit='Minute'):
    '''
...
def save_metrics(alphas, net, net2, d, features, subset, input, output,
    ...
    colnames = 'ratio_routed,tt_non_routed,tt_routed,tt,tt_local,tt_non_local,'
