...
def chicago_parametric_study():
    ...
    # fs = solver_2(g_nr, d, max_iter=1000, q=50, display=1)    
    # np.savetxt('data/test_1.csv', fs, delimiter=',')
    ...
    # fs = solver_3(g_r, d, max_iter=1000, q=50, display=1, stop=1e-2)    
    # np.savetxt('data/test_5.csv', fs, delimiter=',')
...
def chicago_parametric_study_2():
    '''
    study the test_*.csv files generated by chicago_parametric_study()
    in particular, display the average costs for each type of users
...
def chicago_metrics_2():
    ...
    for alpha in [.1,.2,.3,.4,.5,.6,.7,.8,.9]:
        print 'non-routed = {}, routed = {}'.format(1-alpha, alpha)
        fs = np.loadtxt('data/test_{}.csv'.format(int(alpha*100)), delimiter=',', skiprows=0)
        f = np.sum(fs, axis=1)
        print cost(f, net).dot(fs[:,0]) / np.sum((1-alpha)*d[:,2])
        print cost(f, net).dot(fs[:,1]) / np.sum(alpha*d[:,2])
        print ratio_subset(f, net, small_capacity)
