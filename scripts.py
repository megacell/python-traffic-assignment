...
def chicago_metrics():
    ...
    print 'only non-routed users' # 37.7787269935
    f = np.loadtxt('data/test_1.csv', delimiter=',', skiprows=0)
    print average_cost(f, net, d)

    alpha = .25 # 30.9907778552, 27.4728885598
    ...
    alpha = .5 # 28.8858797838, 27.4965806277
    print 'avg tt for {} non-routed and {} routed'.format(1-alpha, alpha)
    ...
    alpha = .75 # 28.175298877, 27.3134702379
    print 'avg tt for {} non-routed and {} routed'.format(1-alpha, alpha)
    ...
    print 'only routed users' # 27.2002445012
...
def chicago_ratio_r_nr():
    ...
    features[:,3] = ratio
    links = process_links(net, node, features)
    color = 5. * ratio # we choose the capacity
